    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/Sravanvihar/calculator-web.git'
            }
        }
        ...
        stage('API Testing') {
            steps {
                script {
                    ...
                    def getResponse = sh(script: 'curl -X GET http://192.168.182.134:8081/webapp-0.2/', returnStdout: true).trim()
                    def postResponse = sh(script: 'curl -X POST -d "n1=5&n2=6&r1=add" http://192.168.182.134:8081/webapp-0.2/firstHomePage', returnStdout: true).trim()
